// Source: https://gist.github.com/stevendesu/2d52f7b5e1f1184af3b667c0b5e054b8
...
function getFractionalBits(n: any) {
...
while (nPrime < 64) {
	...
	// a few bytes
	var isPrime = true;
	// Math.sqrt() was replaced with Math.pow(n, 1/2) to save a few bytes
	// var sqrtN = Math.pow(n, 1 / 2);
...
function convertEndian(word: any) {
	if (LittleEndian) {
		return (
			// byte 1 -> byte 4
			(word >>> 24) |
...
function rightRotate(word: any, bits: any) {
...
function hmac(key: any, data: any): Uint8Array {
	if (key.length > 64)
		key = sha256(key);
...
export function hmac256_sign(inputKey: ArrayBuffer, inputData: ArrayBuffer): Uint8Array {
	//const key = typeof inputKey === "string" ? encoder.encode(inputKey) : inputKey;
	//const data = typeof inputData === "string" ? encoder.encode(inputData) : inputData;
