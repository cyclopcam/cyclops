...
func (c *ConfigDB) NumVerifiedIdentities() (int, error) {
	count := int64(0)
	err := c.DB.Table("user").Where("coalesce(external_id, '') != ''").Count(&count).Error
...
func GetVerifiedIdentityFromToken(token string) (*VerifiedIdentity, error) {
	r, _ := http.NewRequest("GET", AccountsUrl+"/api/auth/checkIdentityToken?token="+token, nil)
...
func (c *ConfigDB) VerifyIdentityAndBindToServer(token string) (*VerifiedIdentity, error) {
	...
	r, _ := http.NewRequest("POST", AccountsUrl+"/api/auth/bindUserToServer/phase1/"+mykey+"?token="+token, nil)
	resp := struct {
	...
	identity := &VerifiedIdentity{}
	r, _ = http.NewRequest("POST", AccountsUrl+"/api/auth/bindUserToServer/phase2/"+mykey+"/"+signature64+"?token="+token, nil)
	err = www.FetchJSON(r, identity)
	if err != nil {
...
func (c *ConfigDB) GetAccountsNonce() int64 {
	key := Key{}
	c.DB.First(&key, "name = ?", KeyAccountsNonce)
	v, _ := strconv.ParseInt(key.Value, 10, 64)
	v++
	key.Name = KeyAccountsNonce
	key.Value = strconv.FormatInt(v, 10)
	if err := c.DB.Save(&key).Error; err != nil {
		c.Log.Errorf("Failed to increment accounts nonce: %v", err)
	}
	return v
}
